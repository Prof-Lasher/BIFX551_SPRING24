...
Tidyverse test file test-drop-na.R
This seems to be the most obvious test to use for na's.
```{r}
...
The least obvious test is Tidyverse test file test-expand.R
On line 334 there is a section that tests to preserve NA's. On line 595 begins a section to test if a function retains NA at the end.
```{r}
...
In the vignette nest.Rmd, the data set is created as a vector within the code. It also uses the mtcars data set as well. There are 4 csv files in the vignette folder which may be for other vignettes.