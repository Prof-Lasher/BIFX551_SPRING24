...
```
Test pivot as listed above is obvious in it's testing. The check_pivot_spec portion with "1" and "mtcars" should produce error. The .name portion should result in an error due to the expectation of using integers, not characters. 

The pivot vignette includes much more detail than I have given. I can appreciate how packages are loaded in their own seperate section as opposed to how I load my packages as needed--although I do that to understand what parts of the code require certain packages. Also uses the titles efficiently to explain what is happening in the proceeding codes. 
...
```
test-chop manipulates dataframes using the chop function by removing specific columns, not chopping columns, preserving grouping and chopping an empty data frame.  
